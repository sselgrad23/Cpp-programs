...
//    XX(int s):m_s{s}{}
    ...
    Doub get(){Doub d_null; d_null.m_a=0;d_null.m_b=0;return d_null;}
    Doub get1(){Doub d_null; d_null.m_a=4;d_null.m_b=2;return d_null;}/*index 2*/
...
Iterator begin(){return Iterator(&m_data[0]);}
Iterator middle(){return Iterator(&m_data[2]);}
...
//std::cout<<dd<<'\n';
	...
	Integers integers;
	std::fill(integers.begin(),integers.middle(),3);
	std::fill(integers.middle(),integers.end(),4);
 ...
 auto ptr {std::find(integers.begin(),integers.end(),3)};/*pointer ptr initialzed at element 2*/ 
// integers.erase(ptr);/*with erase pointer points to next element*/
	for(auto i: xx)std::cout<< i<<'\n';
// ptr =std::find(ptr,v.end(),2);/*second 2 */
// v.erase(ptr);

//std::fill(xx.begin(),xx.end(),xx.get());
